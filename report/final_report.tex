...
\author{ZHU Yulei \and YE K\'evin}
\date{29 mars 2026}
...
Dans ce projet, nous \'etudions plusieurs mani\`eres de mesurer la polarisation d'un profil \'electoral. L'id\'ee g\'en\'erale est de comparer des profils peu polaris\'es, dans lesquels les votantes restent proches d'un m\^eme centre, \`a des profils fortement polaris\'es, dans lesquels l'\'electorat se s\'epare en deux blocs oppos\'es.
...
Ce sch\'ema a l'avantage d'\^etre simple \`a param\'etrer, facile \`a expliquer et surtout compatible avec l'intuition demand\'ee dans le sujet : une faible polarisation correspond \`a un regroupement autour d'un centre unique, tandis qu'une forte polarisation correspond \`a l'apparition de deux p\^oles oppos\'es.
...
\paragraph{Justification du choix}
Le recours \`a l'ordre inverse comme second p\^ole est naturel, car il fournit une opposition forte et facile \`a interpr\'eter. Le bruit introduit par des \'echanges al\'eatoires permet d'obtenir des profils plus r\'ealistes sans rompre la logique globale de polarisation.
...
Notre impl\'ementation suit directement cette d\'efinition dans \texttt{src/pairwise.py} et \texttt{src/phi2.py}. Les exp\'eriences montrent que la mesure cro\^\i t globalement lorsque le niveau de polarisation augmente.
...
Dans le cas ranking, on peut construire des profils o\`u chaque paire de candidates est partag\'ee \'equitablement sans que le profil soit exactement de la forme \(p_{\sigma,\bar{\sigma}}\). La valeur \(1\) traduit donc avant tout un \'equilibre parfait au niveau paire \`a paire, et pas n\'ecessairement une structure strictement bipolaire unique.
...
Dans les deux cadres, \(\phi_2\) r\'eagit dans le sens attendu au param\`etre de polarisation. Cela conforte \`a la fois la m\'ethode de g\'en\'eration retenue et l'impl\'ementation de la mesure.
...
Ces deux fonctions sont impl\'ement\'ees dans \texttt{src/distances.py} sous les noms suivants :
\begin{itemize}[leftmargin=*]
...
Intuitivement, plus l'\'electorat se s\'epare en deux blocs, plus le gain apport\'e par l'utilisation de deux centro\"\i des devient important.

\subsection*{Neutralit\'e}

Si l'on raisonne sur la version id\'eale de la mesure, o\`u \(u_2\) est la vraie valeur optimale du probl\`eme \`a deux centro\"\i des, alors la neutralit\'e est v\'erifi\'ee. Une permutation des candidates ne modifie pas les distances entre bulletins : elle se contente de renommer les candidates. Les valeurs \(u_1\) et \(u_2\) sont donc inchang\'ees, et la mesure aussi.
...
Pour des profils parfaitement bipolaires, le consensus \`a deux centro\"\i des devient nettement plus performant que le consensus global, et la mesure prend alors une valeur \'elev\'ee. Dans le cas approval, le profil \(p_{a,\bar a}\) donne bien la valeur \(1\) pour la version id\'eale de \(\phi_{d_H}\).
...
\end{itemize}
La discussion des cas extr\^emes de r\'egularit\'e est toutefois plus simple dans le cadre approval que dans le cadre ranking.
...
\paragraph{Remarque}
Cette solution est exacte et suffisante pour les tailles de \(m\) utilis\'ees dans nos exp\'eriences. Elle reste fid\`ele \`a l'esprit de l'indication du sujet, m\^eme si nous n'utilisons pas directement \texttt{scipy}.
...
La courbe est nettement croissante. La moyenne passe d'environ \(0.055\) \`a \(0.715\), ce qui montre que, plus le profil est polaris\'e, plus un consensus \`a deux blocs devient pr\'ef\'erable \`a un consensus global unique.
...
La moyenne passe d'environ \(0.125\) \`a \(0.501\). La tendance est globalement croissante. La fin de la courbe reste un peu plus bruit\'ee que dans le cas approval, mais le signal demeure coh\'erent avec l'intuition de polarisation.
...
Les prompts utilis\'es peuvent \^etre regroup\'es en trois grandes cat\'egories :
\begin{itemize}[leftmargin=*]
    \item prompts de g\'en\'eration de code, pour proposer une premi\`ere version de fonctions ou de scripts ;
    \item prompts de d\'ebogage et de validation, pour analyser des erreurs, comparer une impl\'ementation au PDF et corriger certaines formules ;
    \item prompts de r\'edaction, pour reformuler du texte, structurer certaines sections du rapport et clarifier des explications.
\end{itemize}

Nous estimons l'usage total \`a une cinquantaine de prompts, r\'epartis de mani\`ere approximative comme suit :
\begin{itemize}[leftmargin=*]
    \item environ 20 prompts de g\'en\'eration de code ;
    \item environ 15 prompts de d\'ebogage ou de v\'erification ;
    \item environ 15 prompts de reformulation ou d'aide \`a la r\'edaction.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Certaines explications r\'edig\'ees automatiquement \'etaient trop affirmatives ou insuffisamment pr\'ecises sur le plan math\'ematique, ce qui imposait une relecture attentive.
    \item Les textes g\'en\'er\'es avaient parfois une tournure trop g\'en\'erique ; ils ont d\^u \^etre r\'ecrits pour mieux correspondre \`a notre travail r\'eel.
...
L'utilisation de l'IA a \'et\'e globalement positive, surtout pour l'organisation du projet et l'acc\'el\'eration du d\'eveloppement. En revanche, elle n'a jamais dispens\'e d'une lecture attentive du sujet ni d'une validation manuelle des d\'efinitions. Son principal risque est de proposer trop rapidement une solution apparemment coh\'erente, mais math\'ematiquement impr\'ecise.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Documentation officielle de Python : \url{https://docs.python.org/3/}.
    \item Documentation officielle de \texttt{matplotlib} : \url{https://matplotlib.org/stable/}.
    \item Documentation officielle de \texttt{pytest} : \url{https://docs.pytest.org/}.
    \item Outils d'IA g\'en\'erative utilis\'es comme assistance technique et r\'edactionnelle : ChatGPT / Codex, avec v\'erification manuelle syst\'ematique des propositions retenues.
\end{itemize}
...
Les r\'esultats obtenus montrent de mani\`ere coh\'erente que les mesures consid\'er\'ees augmentent lorsque le param\`etre de polarisation \(\alpha\) augmente. Le projet fournit ainsi une impl\'ementation compl\`ete des principales m\'ethodes demand\'ees dans le sujet, ainsi qu'un ensemble d'exp\'eriences permettant d'en comparer le comportement dans les cadres approval et ranking. Il met \'egalement en \'evidence, de mani\`ere coh\'erente, que les diff\'erentes mesures consid\'er\'ees capturent bien l'augmentation progressive du niveau de polarisation.
